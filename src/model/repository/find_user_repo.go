...
func (ur *userRepository) FindUserByNickname(
	nickname string,
) (model.UserDomainInterface, *rest_err.RestErr) {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			errorMessage := fmt.Sprintf("User not found with this nickname: %s", nickname)
			logger.Error(errorMessage,
				err,
				zap.String("journey", "findUserByNickname"))
			return nil, rest_err.NewNotFoundError(errorMessage)
		}
		errorMessage := "Error trying to find user by nickname."
		logger.Error(errorMessage,
			err,
			zap.String("journey", "findUserByNickname"))
		return nil, rest_err.NewInternalServerError(errorMessage, []rest_err.Causes{})
